...
[PEP 517][PEP 517] and [PEP 518][PEP 518] as a standard way to define a Python project.
All tools that build, install, and publish Python packages are expected to use it, but
as of the time of writing, some (including `setuptools`) have yet to fully implement the
proposals.
...
tool installed to run the install program -- see
[the previous section][#pyprojecttoml-and-circular-build-requirements] for more detail.
...
synonyms for some metadata entries, and additional tool-specific metadata.
