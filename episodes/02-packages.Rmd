...
By bringing `SIR_model`, `SEIR_model` and `SIS_model` are brought into the local
namespace in `models/__init__.py`, they are brought into the namespace of
`epi_models.models` too:
...
mark as private using a preceeding underscore. If we set `__all__`, only those objects
with names matching the strings contained within `__all__` will be loaded. For example,
