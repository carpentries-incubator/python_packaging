...
`prproject.toml`, and optionally `setup.py`. However, if you ever find yourself looking
for online help on a Python packaging topic, you'll likely find a large number of
alternative methods being recommended. Some say you should only use `setup.py`, while
others recommend a combination of `setup.cfg` and `setup.py` -- some even say to use
both these and `pyproject.toml`! 
...
provide some context to the often confusing information out there, and to you help if 
you ever find yourself working on a legacy project that hasn't yet updated to the latest
systems.
